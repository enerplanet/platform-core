...
func NewTaskProcessor(db *gorm.DB, cpuThreshold float64) *TaskProcessor {
	return &TaskProcessor{db: db, cpuThreshold: cpuThreshold}
}

func (p *TaskProcessor) ProcessTask(ctx context.Context, t *asynq.Task) error {
	switch t.Type() {
	case jobs.TypeDispatchModelCalculation:
		return jobs.HandleDispatchModelCalculation(ctx, t, p.db, p.cpuThreshold)
	default:
